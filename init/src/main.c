...
void spawn_hello_world(const uint64_t proc_handle,
  ...
  if (sys_proc_mmap(proc_handle, stack_base, stack_size,
                    SYS_PROT_READ | SYS_PROT_WRITE, 0, &stack_base)) {
  ...
  void *ipc_ptr = NULL;
  if (sys_proc_mmap(proc_handle, ipc_base, ipc_size,
                    SYS_PROT_READ | SYS_PROT_WRITE, SYS_MMAP_FL_MIRROR,
                    (uintptr_t *)&ipc_ptr)) {
  ...
  uint64_t task_id;
  if (sys_proc_spawn(proc_handle, 0, stack_top, entrypoint_addr, ipc_base,
                     &task_id)) {
...
int main(struct boot_info *boot_info) {
  ...
  tar_find_file(boot_info->initrd_start, boot_info->initrd_size,
                "bin/hello_world", &hello_world_elf, &hello_world_elf_size);
  if (hello_world_elf) {
    print("Found hello_world in initrd!\n");
    uintptr_t v_entrypoint_addr;
    uint64_t proc_handle;
    if (elf_load(hello_world_elf, hello_world_elf_size, &v_entrypoint_addr,
                 &proc_handle)) {
